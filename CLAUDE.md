...
- **Live progress** works because nodes call `workflow/events.emit(...)` → LangGraph `get_stream_writer()`, and `stream_batch()` iterates `batch_graph.stream(..., stream_mode=["custom","values"], subgraphs=True)` on the Streamlit main thread; `ui/progress.ProgressDashboard.handle(event)` updates placeholders per event. Never touch Streamlit from worker threads. Event shape: `ProgressEvent(candidate_id ("" = batch), stage, status ∈ started|completed|warning|failed|duplicate, message, data)`. The candidate-level `completed` event carries the score fields the live ranking table needs.
